<template>
  <div>
    <v-row>
      <v-col md="4">
        <h2 class="text-center mb-5">Recently Read</h2>
        <v-row v-for="(item,index) in recentBooks" :key="index">
          ...
          >
            <template v-slot:button>
              ...
              <v-btn @click.stop="edit(item,index)">Update</v-btn>
              <v-btn @click.stop="remove(item.category, index)">Delete</v-btn>
            </template>
          </BookCard>
        </v-row>
      </v-col>
      <v-col md="4">
        <h2 class="text-center mb-5">Favourite </h2>
...
export default {
  ...
  created() {
    eventBus.$on("save-book", cardData => {
      if (cardData.category === "Recently read ") {
        this.recentBooks.push(cardData);
        this.recentBooks.sort((a, b) => b - a);
      }
      if (cardData.category === "Favourite ") {
  ...
  methods: {
    ...
    edit(item, index) {
      if (item.category === "Recently read") {
